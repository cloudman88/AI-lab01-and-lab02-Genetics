...
namespace Genetics
{
    ...
    class NQueens : GeneticsAlgorithms<NQueensGen>
    {
        ...
        public NQueens(int n, MutationOperator mutationOperator )
        {
            _n = n;
            _mutationOperator = mutationOperator;
        }
        public override void init_population(ref List<NQueensGen> population, ref List<NQueensGen> buffer)
        {
        ...
        public override void calc_fitness(ref List<NQueensGen> population)
        {
            for (int i = 0; i < GaPopSize; i++)
            {
                uint fitness = 0;
                //fitness += RawFit(population[i].NQueensPos);
                fitness += CrossFit(population[i].NQueensPos);                
                population[i].Fitness = fitness;
            }
        }
        //private uint RawFit(int[] nQueensGen)
        //{
        //    uint rawFit = 0;
        //    var collectionWithDistinctElements = nQueensGen.Distinct().ToArray();
        //    uint dif = (uint)Math.Abs(nQueensGen.Count() - collectionWithDistinctElements.Length);
        //    if (dif != 0) rawFit += 10* dif;
        //    return rawFit;
        //}
        private uint CrossFit(int[] nQueensGen)
        {
            uint crossFit = 0;
            for (int i = 0; i < _n; i++)
            {
        ...
        public override void Mutate(NQueensGen member)
        {            
            List<int> nQlist = member.NQueensPos.ToList();
            List<int> chosenPart = null;
            ...
            switch (_mutationOperator)
            {
                    ...
                    case MutationOperator.Inversion:
                        ipos2 = Rand.Next() % (_n - ipos1) + ipos1;
                        int[] temp = MutOpSimpleInversion(ipos1, ipos2, nQlist);
        ...
        public int[] MutOpDisplacement(int ipos1,int ipos2, List<int> nQlist)
        {
            ...
            return theRest.ToArray();
        }
        public int[] MutOpExchange(int ipos1, int ipos2, List<int> nQlist)
        {
            int temp = nQlist[ipos1];
            nQlist[ipos1] = nQlist[ipos2];
            nQlist[ipos2] = temp;
            return nQlist.ToArray();
        }
        public int[] MutOpInsertion(int ipos1, int ipos2, List<int> nQlist)
        {
        ...
        public int[] MutOpSimpleInversion(int ipos1, int ipos2, List<int> nQlist)
        {
            List<int> chosenPart = new List<int>();
        ...
        public int[] MutOpScramble(int ipos1, int ipos2, List<int> nQlist)
        {
            List<int> chosenPart = new List<int>();
            for (int i = ipos1; i < ipos2 + 1; i++)
            {
        ...
        public override void MateByMethod(NQueensGen bufGen, NQueensGen gen1, NQueensGen gen2,
            CrossoverMethod method)
        {
            List<int> nQlist1 = gen1.NQueensPos.ToList();
            List<int> nQlist2 = gen2.NQueensPos.ToList();
            int ipos1 = Rand.Next() % _n;
            //int qpos2 = Rand.Next() % (_n - qpos1) + qpos1;
            switch (method)
            {
                case CrossoverMethod.PMX:
                    ...
                    nQlist1[ipos1] = val2;
                    nQlist2[ipos1] = val1;
                    bufGen.NQueensPos = nQlist2.ToArray();
                    break;
                case CrossoverMethod.OX:
                    ...
                    bufGen.NQueensPos = temp.ToArray();
                    break;
                case CrossoverMethod.CX:
                    ...
                    bufGen.NQueensPos = result.ToArray();
                    break;
                case CrossoverMethod.ER:
                    ...
                    bufGen.NQueensPos = res.ToArray();

                    break;
            }
        }

        public void RemoveNeigbhorFromAllLists(Dictionary<int, List<int>> neighborsDicMerged,int target)
        {
            for (int i = 0; i < _n; i++)
            {
        ...
        public override Tuple<string, uint> GetBestGenDetails(NQueensGen gen)
        {
            string str = "";
