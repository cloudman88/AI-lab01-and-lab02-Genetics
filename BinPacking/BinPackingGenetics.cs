...
namespace Genetics.BinPacking
{
    class BinPackingGenetics : GeneticsAlgorithms<VolumesGen>
    {
        ...
        private ObservableCollection<List<int>> _resultBins;
        private BinPackingAlgorithm bpa;
        ...
        protected override void calc_fitness()
        {
            ...
            for (int i = 0; i < GaPopSize; i++)
            {
                ...
                if (minFit>=Population[i].Fitness) // save the best result bins so far
                {
                    minFit = Population[i].Fitness;
                    _resultBins = new ObservableCollection<List<int>>(bpa.Bins);
                }
            }
        }
        ...
        protected override VolumesGen get_new_gen()
        {
        ...
        protected override int calc_distance(VolumesGen gen1, VolumesGen gen2)
        {
            ...
            int distance = 0; 
            var volumes1 = gen1.Volumes;
            var volumes2 = gen2.Volumes;
            for (int i = 0; i < _volumes.Count; i++)
            {
